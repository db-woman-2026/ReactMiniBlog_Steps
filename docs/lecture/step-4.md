...
개인 저장소의 `main`에 Step 3 결과를 commit하고 GitHub에 push한 상태로 시작합니다.
...
서로 다른 author 값을 전달한 두 `PostCard`의 출력 차이를 확인합니다. 결과와 확인 방법을 한 문장으로 기록합니다. 실험값은 검사를 마치면 원래대로 복구합니다.
