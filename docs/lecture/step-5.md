...
게시글 하나만 있던 데이터를 여러 게시글이 들어 있는 배열로 바꿉니다. 각 게시글에는 상세 페이지에서 사용할 `id`와 `content`도 함께 둡니다.
...
게시글 객체 하나를 추가하고 `key`가 중복되지 않는지 확인합니다. 결과와 확인 방법을 한 문장으로 기록합니다. 실험값은 검사를 마치면 원래대로 복구합니다.
